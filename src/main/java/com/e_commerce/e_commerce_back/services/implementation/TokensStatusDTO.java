...
public class TokensStatusDTO {
    ...
    private int getExpiryMinutesForType(String type) {
        switch (type) {
            ...
            case "email_change":
                return activationCodeExpiryMinutes; // Mismo tiempo que activation (15 min)
            default:
                return 15;
        }
    }

    // ================= EMAIL CHANGE CODE (NUEVO) =================

    /**
     * Genera y almacena código de cambio de email
     * Almacena temporalmente el nuevo email junto con el código
     * 
     * @param userId ID del usuario
     * @param newEmail Nuevo email a cambiar
     * @return Código de 6 dígitos
     */
    public String generateAndStoreEmailChangeCode(Long userId, String newEmail) {
        String code = generateSecureCode(CODE_LENGTH);
        String key = EMAIL_CHANGE_PREFIX + userId;
        
        // Almacenar código:newEmail como valor
        String value = code + ":" + newEmail;
        redisTemplate.opsForValue().set(key, value, Duration.ofMinutes(activationCodeExpiryMinutes));
        
        // Mapeo inverso para búsqueda por código
        storeCodeToUserIdMapping(code, userId, "email_change");
        
        log.info("Código de cambio de email generado para usuario: {} -> nuevo email: {}", userId, newEmail);
        return code;
    }

    /**
     * Verifica código de cambio de email (sin consumir)
     * 
     * @param userId ID del usuario
     * @param code Código a verificar
     * @return true si el código es válido
     */
    public boolean isEmailChangeCodeValid(Long userId, String code) {
        String key = EMAIL_CHANGE_PREFIX + userId;
        ...
        return parts.length == 2 && parts[0].equals(code.trim());
    }

    /**
     * Verifica y consume código de cambio de email
     * Retorna el nuevo email si el código es válido
     * 
     * @param userId ID del usuario
     * @param code Código de verificación
     * @return Nuevo email si es válido, null si no
     */
    public String verifyAndConsumeEmailChangeCode(Long userId, String code) {
        log.info("Verificando código de cambio de email para usuario: {}", userId);
        
        String mainKey = EMAIL_CHANGE_PREFIX + userId;
        String storedValue = redisTemplate.opsForValue().get(mainKey);
        
        if (storedValue == null) {
            log.warn("No hay solicitud de cambio de email pendiente para usuario: {}", userId);
            return null;
        }
        
        String[] parts = storedValue.split(":");
        if (parts.length != 2) {
            log.error("Formato inválido de datos de cambio de email para usuario: {}", userId);
            redisTemplate.delete(mainKey);
            return null;
        }
        ...
        if (!code.trim().equals(storedCode)) {
            log.warn("Código de cambio de email incorrecto para usuario: {}", userId);
            return null;
        }
        
        // Eliminar código y mapeo
        redisTemplate.delete(mainKey);
        String mappingKey = getMappingKey("email_change", code.trim());
        redisTemplate.delete(mappingKey);
        
        log.info("Código de cambio de email verificado exitosamente para usuario: {} -> nuevo email: {}", 
            userId, newEmail);
        return newEmail;
    }

    /**
     * Obtiene el nuevo email pendiente de verificación
     * 
     * @param userId ID del usuario
     * @return Nuevo email pendiente o null
     */
    public String getPendingEmailChange(Long userId) {
        String key = EMAIL_CHANGE_PREFIX + userId;
        String storedValue = redisTemplate.opsForValue().get(key);
    ...
    public void cancelEmailChange(Long userId) {
        ...
        if (storedValue != null) {
            ...
            if (parts.length == 2) {
                String code = parts[0];
                String mappingKey = getMappingKey("email_change", code);
                redisTemplate.delete(mappingKey);
            }
        }
        
        redisTemplate.delete(key);
        log.info("Solicitud de cambio de email cancelada para usuario: {}", userId);
    }

    /**
     * Verifica si hay una solicitud de cambio de email pendiente
     * 
     * @param userId ID del usuario
     * @return true si hay una solicitud pendiente
     */
    public boolean hasActiveEmailChangeRequest(Long userId) {
        return Boolean.TRUE.equals(redisTemplate.hasKey(EMAIL_CHANGE_PREFIX + userId));
    }

    /**
     * Obtiene TTL de la solicitud de cambio de email
     * 
     * @param userId ID del usuario
     * @return Minutos restantes o -1 si no existe
     */
    public long getEmailChangeCodeTTL(Long userId) {
        Long ttl = redisTemplate.getExpire(EMAIL_CHANGE_PREFIX + userId, TimeUnit.MINUTES);
        return ttl != null ? ttl : -1;
    }

    /**
     * Reenvía código de cambio de email con rate limiting
     * 
     * @param userId ID del usuario
     * @param newEmail Nuevo email
     * @return Nuevo código
     */
    public String resendEmailChangeCodeWithRateLimit(Long userId, String newEmail) {
        if (!canRequestToken(userId, "email_change")) {
            Duration remaining = getRateLimitTimeRemaining(userId, "email_change");
            throw new RuntimeException(String.format(
                "Demasiados intentos. Intente nuevamente en %d minutos",
                remaining.toMinutes()));
        }
        
        cancelEmailChange(userId);
        return generateAndStoreEmailChangeCode(userId, newEmail);
    }
